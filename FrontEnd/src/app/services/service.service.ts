...
export class ServiceService {
	...
	get(uuid : string) : Observable<Service> {
		...
		return this.http.get('api/service', { search : params })
      .map((result) => result.json());
	}
	...
	delete(uuid : string) : Observable<any> {
		...
		return this.http.delete('api/service/', { search : params });
	}

	deleteData(serviceUuid : string) : Observable<any> {
		let params = new URLSearchParams();
		params.set('serviceId', serviceUuid);
		return this.http.delete('api/service/data', { search : params });
	}
	...
	getServiceOrganisations(uuid : string) : Observable<Organisation[]> {
		let params = new URLSearchParams();
		params.set('uuid',uuid);
		return this.http.get('api/service/organisations', { search : params })
      .map((result) => result.json());
	}
	...
	getSystemsForService(serviceId : string) : Observable<System[]> {
		...
		return this.http.get('api/service/systemsForService', { search : params })
      .map((result) => result.json());
	}
