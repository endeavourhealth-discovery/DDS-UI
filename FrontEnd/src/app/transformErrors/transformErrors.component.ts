...
@Component({
	templateUrl : './transformErrors.html'
})
export class TransformErrorsComponent {
	...
	refreshSummaries() {
		...
		vm.exchangeAuditService.getTransformErrorSummaries()
			.subscribe(
				(result) => {
					vm.transformErrorSummaries = result.sort((a,b) => a.service.name.toLowerCase().localeCompare(b.service.name.toLowerCase()));
					vm.applyFiltering();
	...
	actionItem(uuid : string, action : string) {
		this.router.navigate(['resources', action, uuid]);
	}
