...
@Component({
	templateUrl : './exchangeAudit.html'
})
export class ExchangeAuditComponent implements OnInit {
  ...
  this.systemId = params['systemId'];
		...
		this.serviceService.get(serviceId)
			.subscribe(
				(result) => {
					this.service = result;
				...
				(error) => this.log.error('Failed to retrieve service', error, 'Refresh Service')
			)
	...
	private loadTransformAudits(exchange: Exchange) {
		...
		vm.exchangeAuditService.getInboundTransformAudits(serviceId, systemId, exchangeId, true).subscribe(
			(result) => {
				...
				exchange.transformAudits = result.sort((a,b) => b.transformStart - a.transformStart);
